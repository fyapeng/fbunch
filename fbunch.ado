*! version 2.0  Date: 2025-11-23
*! Title: fbunch - Data-driven Bunching Estimator
*! Author: Easton Fu & You
*! Description: Bunching estimator with auto-window/degree selection, round-number correction, and outcome analysis.

capture program drop fbunch
program define fbunch, rclass
    version 14.0
    syntax varname(numeric), Cutoff(real) [ ///
        Width(real 0.0)       /// Bin width (0=Auto)
        Degree(int 0)         /// Polynomial degree (0=Auto)
        Maxdeg(int 7)         /// Max degree for auto-selection
        Select(string)        /// Selection criteria: mse, aic, bic
        IMprove(real 0.05)    /// Elbow rule threshold
        Window(numlist min=2 max=2) /// Manual excluded window
        Model(string)         /// Model type: kink or notch
        Side(string)          /// Bunching side: left or right
        Constraint            /// Notch constraint (B=M)
        Searchrange(real 0)   /// Range to search for vanishing point (Notch)
        BALance(string)       /// Integration constraint: left or right
        Outcome(varname)      /// Outcome variable for causal analysis
        Reps(int 0)           /// Bootstrap repetitions
        Gen(string)           /// Prefix for generated variables
        Noplot                /// Suppress plotting
        Seed(int 0)           /// Random seed
        TOLerance(real 0.02)  /// Convergence tolerance
        Round(numlist)        /// Round number cycles (e.g., 10 50)
    ]
    ...
    * ==========================================================================
    quietly {
        ...
        * Freedman-Diaconis rule for bandwidth
        if `width' == 0 {
    ...
    * ==========================================================================
    quietly _fbunch_core, cutoff(`cutoff') width(`width') ///
        ...
        has_outcome("`outcome'") tolerance(`tolerance') round("`round'")
    ...
    * ==========================================================================
    ...
    if `reps' > 0 {
        ...
        quietly {
            ...
            * Save counterfactuals for resampling base
            keep bin_center cf_freq cf_out_hat `poly_list'
            save `base_cf', replace
            
            * Pool residuals from non-excluded region
            use `data_main_results', clear
        ...
        if `do_boot' {
            ...
            forvalues i = 1/`reps' {
                quietly {
                    ...
                    * Re-estimate (using fixed degree and window)
    ...
    * Calculate P-value for Notch Constraint (H0: B = M)
    ...
    * ==========================================================================
    ...
    di as txt "FBUNCH Estimation Results (v2.0)"
    di as txt "Model: " as res upper("`model'") " (" upper("`side'") ")" _col(45) as txt "Total Obs: " as res %12.0f `total_N'
    if "`round'" != "" di as txt "Integer Adj.: " as res "Active (Cycles: `round')"
    di as txt "{hline 72}"
    ...
    * --- Return Scalars ---
    ...
    if "`model'" == "notch" return scalar p_val_notch = `p_val'
...
program define _fbunch_core, rclass
    syntax, Cutoff(real) Width(real) [Degree(int 0) Maxdeg(int 7) Select(string) Improve(real 0.05) Window(string) Model(string) Side(string) Constraint Searchrange(real 0) has_outcome(string) Tolerance(real 0.02) Balance(string) Round(string)]
    ...
    * Generate Round Number Dummies
    local round_vars ""
    if "`round'" != "" {
        foreach r of numlist `round' {
            capture drop d_round_`r'
            tempvar orig_val 
            gen double `orig_val' = bin_center + `cutoff'
            * Detect if bin falls on a multiple of r
            gen byte d_round_`r' = 0
            replace d_round_`r' = 1 if abs(`orig_val'/`r' - round(`orig_val'/`r')) * `r' < (`width'/2 + 1e-5)
            local round_vars "`round_vars' d_round_`r'"
        }
    }
    ...
    * 1. Initialize Window
    ...
    if `fixed_window' {
        local conv = 1
        if `degree' == 0 {
             _fbunch_select_deg, lower(`z_lower') upper(`z_upper') max(`maxdeg') sel(`select') imp(`improve') extra_vars("`round_vars'")
    ...
    * 2. Iterative Selection Loop
    while `conv' == 0 {
        ...
        * Regress (Polynomials + Integer Dummies)
        quietly reg freq `reg_vars' `round_vars' if bin_center < `z_lower' | bin_center > `z_upper'
    ...
    local final_vars "`final_vars' `round_vars'"

    * 3. Notch Constraint Search
...
* ==============================================================================
* SUBROUTINE 2: DEGREE SELECTION (5-Fold CV)
* ==============================================================================
capture program drop _fbunch_select_deg
program define _fbunch_select_deg, rclass
    syntax, lower(real) upper(real) max(int) sel(string) imp(real) [extra_vars(string)]
    ...
    forvalues p = 1/`max' {
        ...
        local press = 0
        forvalues k=1/5 {
            quietly reg freq `vars' `extra_vars' if `cv_grp'!=. & `cv_grp'!=`k'
            tempvar p_cv
        ...
        local N_eff = r(N)
        local n_extra : word count `extra_vars'
        local k_param = `p' + 1 + `n_extra'
